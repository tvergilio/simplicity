...
//Tweaked the median implementation so Spock tests will run.
...
class BrokenCode3 {
    ...
    BigDecimal median(Collection<Integer> numbers) {
        if (numbers.size() % 2 == 0) {
            ...
            def firstIndex = Math.floor(numbers.size() / 2) - 1
            return (BigDecimal) mean(numbers.sort()[firstIndex..(firstIndex + 1)])
        } else {
            def index = Math.floor(numbers.size() / 2)
            return numbers.sort().get((int)index)
        }
    }
